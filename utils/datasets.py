...
class SeismicDataset1D(Dataset):
	...
	def __getitem__(self, index):
		...
		x = torch.tensor(self.seismic[trace_index, :, :], dtype=torch.float)
		y = torch.tensor(self.model[trace_index, :, :], dtype=torch.float)
		return x, y
...
class UnsupervisedSeismicDataset(Dataset):
	...
	def __getitem__(self, index):
		trace_index = self.trace_indices[index]
		# x = torch.tensor(self.seismic[trace_index][np.newaxis, :], dtype=torch.float)
		x = torch.tensor(self.seismic[trace_index], dtype=torch.float)
...
class UnsupervisedSeismicDataset_unpaire(Dataset):
	...
	def __getitem__(self, index):
		...
		y = torch.tensor(self.model[tindex%len(self.model)], dtype=torch.float)
		x = torch.tensor(self.seismic[tindex], dtype=torch.float)
...
class SeismicDataset1D_SPF(Dataset):
	...
	def __getitem__(self, index):
		...
		x = torch.tensor(self.seismic[trace_index, :, :], dtype=torch.float)
		y = torch.tensor(self.model[trace_index, :, :], dtype=torch.float)
		z = torch.tensor(self.facie[trace_index, 0, :], dtype=torch.long)
		return x, y, z
...
class SeismicDataset1D_SPF_WS(Dataset):
	"""
		...
		返回：（channel, depth_samples）
	"""
	def __init__(self, seismic, facie, trace_indices, prior=None):
		# 直接把所有的数据读入
		self.seismic = seismic
		self.facie = facie
		self.prior = prior
	...
	def __getitem__(self, index):
		...
		if self.prior is None:
			return x, z
		p = torch.tensor(self.prior[trace_index, :, :], dtype=torch.float)
		return x, z, p
...
class SeismicDataset2D(Dataset):
	...
	def __getitem__(self, index):
		...
		trace_index = self.trace_indices[index]
		x = torch.tensor(self.seismic[trace_index-offset:trace_index+offset+1].T[np.newaxis, :, :], dtype=torch.float)
		y = torch.tensor(self.model[trace_index][np.newaxis, :], dtype=torch.float)
		return x, y
